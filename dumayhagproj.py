...
def submit():
    conn=sqlite3.connect('C:/Users/STUDENTS.DESKTOP-65RJPDU/Documents/myproject.db')
    c=conn.cursor()

    c.execute("INSERT INTO customer_info VALUES(:f_name,:l_name,:age,:address,:email)",
              {
                  'f_name': f_name.get(),
                  'l_name':l_name.get(),
                  'age':age.get(),
...
def query():
    conn=sqlite3.connect('C:/Users/STUDENTS.DESKTOP-65RJPDU/Documents/myproject.db')
    c=conn.cursor()
    c.execute("SELECT *,oid FROM myproject.db")
    records=c.fetchall()
...
def delete():
    ...
    c.execute("DELETE from customerinfo WHERE oid="+delete_box.get())
...
def edit():
    ...
    c.execute("SELECT * FROM customerinfo WHERE oid ="+record_id)
    record=c.fetchall()
...
'''
c.execute ("""CREATE TABLE "Costumer_info" (
	"f_name"	TEXT,
	"l_name"	TEXT,
	"age"	INTEGER,
	"address"	TEXT,
